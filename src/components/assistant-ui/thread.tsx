  ...
  const suggestions = [
    {
      ...
      title: "🌦️ Check Weather in Paris",
      description: "Find out live temperature and conditions."
    },
    {
      prompt: "Analyze AAPL stock price",
      title: "📈 Analyze AAPL Stock",
      description: "Retrieve current valuation and trend metrics."
    },
    {
      prompt: "Search web for React Vite styling templates",
      title: "🔍 Search for Vite Templates",
      description: "Perform web lookup with citations and summaries."
    }
  ];
  ...
  return (
    <div className="aui-thread-welcome-suggestions grid w-full @md:grid-cols-2 gap-2 pb-4">
      {suggestions.map((suggestion, index) => (
        <div key={index} className="aui-thread-welcome-suggestion-display fade-in slide-in-from-bottom-2 animate-in fill-mode-both duration-200">
          <Button
            variant="ghost"
            onClick={() => handleSuggestionClick(suggestion.prompt)}
            className="aui-thread-welcome-suggestion h-auto w-full flex flex-col items-start justify-start gap-1 rounded-3xl border bg-background px-4 py-3 text-start text-sm transition-colors hover:bg-muted"
          >
